...
| 범주 | 기능 |
|---|---|
| **터미널 및 연결** | 로컬 셸, SSH, Mosh, Telnet, 시리얼, 분할 창, 자유 입력 모드, 구성 가능한 세션 로그, 창 레이아웃 및 드래그 가능한 구분선을 갖춘 네이티브 tmux -CC 제어 모드, 이름이 지정된 브로드캐스트 그룹, 여러 대상에 대한 고급 명령 전송, 멀티홉, 안정적인 재연결 |
...
[최신 릴리스 다운로드](https://github.com/AnalyseDeCircuit/oxideterm/releases/latest)
